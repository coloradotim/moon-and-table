...
The content packet workflow lives in `docs/content-packets/README.md`. Packet status controls whether proposed content can become active:

- `draft` and `ready_for_review` packets are review artifacts only.
- `approved_for_implementation` packets can support later active content PRs.
- `implemented` packets have already been used by later implementation work.
- `rejected` packets cannot be used without new review.

Draft packet proposals must not be treated as active SourceReviews, SourceNotes, SymbolicCards, RitualPatterns, RitualPresentations, RitualMeaningBridges, scoring rules, generator behavior, or UI categories.
...
The broader timing fact API lives in `src/lib/timing-facts.ts` and currently computes lunar phase, lunations, moon sign, sun sign, seasonal markers, planetary signs, retrograde status, major aspects, universal numerology date facts, and household calendar threshold facts for first day, last day, and month turn. Date-derived facts use the supplied timezone, or the runtime local timezone by default; first day, last day, month turn, and numerology day/month/year are household-local calendar concepts, not UTC calendar labels. Universal numerology facts come from `src/lib/numerology-timing.ts`, which computes universal year, month, and day numbers by reducing date sums to 1-9; master numbers are reduced for MVP. Calendar threshold facts are computed by the app calendar layer; they are not schedule awareness, named holiday content, or folklore instructions. The first rule layer lives in `src/lib/timing-interpretation-rules.ts`; approved rules cover lunar phase cards, numerology cards, four solstice/equinox seasonal anchors, MVP astrology cards for Sun through Saturn, all 12 signs, five major aspects, conservative retrograde cue, and first/last/month-turn household threshold support. Weather-aware seasonal interpretation, local ecology, cross-quarter days, named-day calendars, and outer-planet interpretation remain deferred until reviewed source cards exist.
