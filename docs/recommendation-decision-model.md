...
The explanation layers must serve meaning, not justify machinery. `Why this
fits` should explain why the ritual answers the check-in through timing,
material/action logic, capacity, and audience. It should not merely say that
the selected context matched. `How this was chosen` can disclose a
closest-compatible match or coverage gap, but it should do that in readable
household-grimoire language, not score, candidate, or broken-app language.
...
Optional accents and carry prompts are presentation decisions, not score reasons.
They should prefer the selected pattern's material/action logic, then
focus-aware or capacity-aware copy, before using any timing fallback. Source
summaries should describe the human-readable lineage that shaped the selected
pattern, not raw source IDs or diagnostic keys.
...
Authored-output scenarios add a second report layer for final-copy quality.
They flag option-menu ritual bodies, title/intention duplication without depth,
`Why this fits` that describes matching instead of meaning,
`How this was chosen` that reads like a system report, raw or academic source
lineage, high capacity without deeper ritual shape, both-of-us copy that only
changes pronouns, ritual bodies without activation or closure, material treated
as a prop, closest-compatible overclaiming, and coverage-gap disclosure that is
missing or written as apology.
